...
var chain_direction := Vector2(0,0)	# The direction in which the chain was shot
...
func shoot(dir: Vector2) -> void:
	$HookShot.play()
	chain_direction = dir.normalized()	# Normalize the direction and save it
...
func _physics_process(_delta: float) -> void:
	...
	if flying:
		...
		if $Tip.move_and_collide(chain_direction * SPEED):
			#$HookHit.play()
